# Welcome to Satvik Pedapalli's Cybersecurity Portfolio

![GitHub Profile Views](https://komarev.com/ghpvc/?username=satvik-pedapalli&color=blue) ![GitHub Stars](https://img.shields.io/github/stars/satvik-pedapalli?style=social) ![Followers](https://img.shields.io/github/followers/satvik-pedapalli?style=social)

---

### **About Me** ![Cybersecurity Professional](https://via.placeholder.com/1200x400?text=Satvik+Pedapalli+|+Cybersecurity+Aspirant)  
Hi! I'm **Satvik Pedapalli**, a **B.Tech graduate (2025)** transitioning into the Cybersecurity domain with a focus on:
- 🛡 **Vulnerability Assessment and Penetration Testing (VAPT)**
- 💻 **Application Security & Static Analysis (SAST)**
- 🔍 **Network Security & Reconnaissance**
- 📑 **Technical Documentation & Reporting**

I am a detail-oriented professional dedicated to identifying security weaknesses and validating vulnerabilities. Having transitioned from a technical engineering background, I bring strong analytical thinking and a continuous learning mindset to the field of information security.

---

### **Core Competencies** ![Cybersecurity Skills](https://via.placeholder.com/1200x400?text=Cybersecurity+Skills+&+Competencies)

- **Vulnerability Assessment**: Practical exposure to identifying and validating security vulnerabilities in alignment with industry standards (OWASP, CVSS).
- **Application Security**: Proficient in performing static code analysis (SAST) and dependency scanning using tools like SonarQube and Snyk.
- **Network Security**: Experienced in network discovery, service enumeration, and reconnaissance using industry-standard tools.
- **Technical Reporting**: Adept at structured vulnerability reporting, risk evaluation, and delivering clear remediation recommendations.
- **Laboratory Research**: Actively practicing real-world attack simulations through Hack The Box and PortSwigger Web Security Academy.

---

### **Key Tools & Technologies**

| **Category** | **Tools** |
|-----------------------|---------------------------------------------------------------------------|
| 🔍 **Penetration Testing** | Burp Suite, Nmap, Wireshark, Metasploit  |
| 📑 **Static Analysis** | SonarQube, Snyk, Semgrep                 |
| 🌐 **Web Standards** | OWASP Top 10                             |
| 🖥 **Operating Systems** | Linux (Kali, Ubuntu), Windows            |

---

### **🚀 Featured Research & Projects**

**1. Static Application Security Testing (SAST)** 📌 *Utilizing SonarQube, Snyk, and Semgrep to identify code-level vulnerabilities.*
- **Focus**: Identifying hardcoded secrets, SQL injection risks, and vulnerable dependencies.
- **Impact**: Enhancing software security posture during the development lifecycle.

**2. Network Discovery & Enumeration** 📌 *Conducting VAPT on network lab environments aligned with industry standards.*
- **Focus**: Performing service enumeration and reconnaissance using Nmap and Wireshark.
- **Impact**: Mapping attack surfaces and identifying misconfigured services.

**3. Web Application Security** 📌 *Practicing web penetration testing based on OWASP Top 10 standards.*
- **Focus**: Utilizing Burp Suite for vulnerability identification and exploitation techniques.
- **Impact**: Strengthening practical skills in identifying web-based security flaws.

---

### **Experience & Education** - 🎓 **B.Tech (Civil Engineering)**: Siddhartha Engineering College, Vijayawada (2025).
- 📊 **Technical Reporting**: Prepared detailed reports including CVSS-based severity classification and remediation guidance.
- 🤝 **Continuous Learning**: Actively pursuing industry-recognized certifications (CEH, ISC2 CC).

---

### **Certifications** - 🎓 **Certified Ethical Hacker (CEH)** - *Pursuing*
- 🎓 **ISC2 Certified in Cybersecurity (CC)** - *Pursuing*

---

### **Contact Me** 💌 **Email**: [pedapallisatvik.2003@gmail.com](mailto:pedapallisatvik.2003@gmail.com)
🔗 **LinkedIn**: [linkedin.com/in/satvik-pedapalli](https://www.linkedin.com/in/satvik-pedapalli) 
🌐 **GitHub**: [github.com/satvik-pedapalli](https://github.com/satvik-pedapalli)
